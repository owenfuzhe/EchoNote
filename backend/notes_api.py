"""
EchoNote Backend - Notes API Module (APIRouter version)
FastAPI + SQLAlchemy
"""
...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./echonote.db")
...
async def list_notes(
    ...
    if search:
        ...
        query = query.filter(
            (NoteDB.title.ilike(search_filter)) |
            (NoteDB.content.ilike(search_filter)) |
            (NoteDB.transcript.ilike(search_filter))
        )
...
@router.post("/notes/{note_id}/transcript", response_model=TranscriptResponse)
async def generate_transcript(
    ...
    note.transcript = "这是语音转录的模拟文本。实际使用时应调用 Whisper API 或本地语音识别服务。"
